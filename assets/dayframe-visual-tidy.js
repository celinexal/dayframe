(() => {
  ...
  function visible(el) {
    ...
    const page = el.closest('.pg,.page,[id^="pg-"]');
    if (page && !page.classList.contains('on') && getComputedStyle(page).display === 'none') return false;
  ...
  function spendingSummary(items, cycle) {
    const rows = {};
    items.forEach((item) => {
  ...
  function overviewRow(row, index) {
    ...
    const status = !row.limit ? 'Set budget' : row.remaining < -0.005 ? `${money(Math.abs(row.remaining))} over` : `${money(row.remaining)} left`;
    ...
    return `
      <button class="df-budget-overview-row ${state}" type="button" onclick="moneyOpenTransactions('${encoded}','cycle')" style="--df-row-index:${index}">
        ...
        <span class="df-budget-overview-main"><strong>${esc(row.category)}</strong><small>${row.count} transaction${row.count === 1 ? '' : 's'} this cycle</small></span>
        <span class="df-budget-overview-amount"><strong>${meta}</strong><small>${esc(trendText(row.spent, row.lastSpent))}</small></span>
  ...
  function renderBudgetOverview() {
    ...
    if (!host || !visible(host) || typeof hubLoad !== 'function') return;
    const d = hubLoad();
    const bankTransactions = safeCall(() => (_moneyBankData.transactions || []), []);
    const cycle = cycleFor(new Date());
    const signature = [
      cycle.start,
      cycle.end,
      (d.budgets || []).map((b) => `${normaliseCategory(b.category)}:${b.limit}`).join('|'),
      (d.money || []).length,
      bankTransactions.length,
      Object.keys(d.transactionCategoryOverrides || {}).length,
    ].join('::');
    if (host.dataset.dfBudgetOverviewSignature === signature && host.classList.contains('df-budget-overview') && host.innerHTML.trim()) return;
    host.dataset.dfBudgetOverviewSignature = signature;
    ...
    const HIDDEN_HERE = new Set(['Other', 'Rent', 'Bills & Utilities', 'Subscriptions']);
    ...
    const rows = categories.map((category) => {
      ...
      return { category, limit, spent, lastSpent, count: currentSpending[category]?.count || 0, remaining: limit - spent, hasBudget: limit > 0 };
    }).sort((a, b) => {
    ...
    const summaryLabel = totalLimit ? totalRemaining < 0 ? `${money(Math.abs(totalRemaining))} over` : `${money(totalRemaining)} left` : `${money(totalSpent)} tracked`;
    ...
    host.innerHTML = `
      ...
      <div class="df-budget-overview-list">${rows.map(overviewRow).join('')}</div>
      <div class="df-budget-overview-foot">${esc(cycleLabel(cycle))} &middot; tap a category to see its transactions.</div>
    `;
  }
  ...
  function ensureStyle() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      #pg-dashboard .invest-learn-bridge.df-invest-learning-quiet{grid-template-columns:minmax(220px,.62fr) minmax(0,1.38fr);gap:14px;align-items:center;margin:0 0 16px;padding:16px 18px;border:1px solid #e7ebf3;border-radius:16px;background:linear-gradient(135deg,#ffffff 0%,#f8fbff 52%,#f7f4ff 100%);color:#1f2937;box-shadow:0 12px 30px rgba(32,40,59,.075)}
      #pg-dashboard .invest-learn-bridge.df-invest-learning-quiet:after{display:none}
      #pg-dashboard .df-invest-learning-quiet .invest-learn-kicker{padding:0;border:0;background:transparent;border-radius:0;font-size:9px;letter-spacing:.08em;color:#7c8799}
      #pg-dashboard .df-invest-learning-quiet .invest-learn-intro h2{margin:5px 0 4px;font-size:18px;line-height:1.2;letter-spacing:0;color:#1f2937}
      #pg-dashboard .df-invest-learning-quiet .invest-learn-intro p{max-width:520px;font-size:10.5px;line-height:1.55;color:#6b7688}
      #pg-dashboard .df-invest-learning-quiet .invest-learn-intro>button{margin-top:10px;padding:7px 10px;border:1px solid #e0e5f1;border-radius:10px;background:#fff;color:#685cf0;font-size:10px;box-shadow:0 4px 12px rgba(32,40,59,.045)}
      #pg-dashboard .df-invest-learning-quiet .invest-learn-links{grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}
      #pg-dashboard .df-invest-learning-quiet .invest-learn-links>button{min-height:72px;padding:11px 12px;border:1px solid #e7ebf3;border-radius:12px;background:rgba(255,255,255,.72);color:#2f394c;box-shadow:none}
      #pg-dashboard .df-invest-learning-quiet .invest-learn-icon{width:27px;height:27px;border-radius:9px;font-size:12px;background:#f2f5fb}
      #pg-dashboard .df-invest-learning-quiet .invest-learn-links strong{color:#263246;font-size:10.5px}
      #pg-dashboard .df-invest-learning-quiet .invest-learn-links small{color:#7a8597;font-size:9px;line-height:1.4}
      #pg-dashboard .df-invest-learning-quiet .invest-learn-links i{color:#98a2b3}
      #money-budget-overview.df-budget-overview{gap:0}
      .df-budget-overview-summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-bottom:12px}
      .df-budget-overview-summary>div{min-width:0;padding:9px 10px;border:1px solid #edf0f6;border-radius:10px;background:#fbfcff}
      .df-budget-overview-summary span{display:block;margin-bottom:3px;color:#98a2b3;font-size:8.5px;font-weight:850;letter-spacing:.08em;text-transform:uppercase}
      .df-budget-overview-summary strong{display:block;color:#263246;font-family:var(--fd);font-size:15px;line-height:1.12;word-break:break-word}
      ...
      .df-budget-overview-list{display:flex;flex-direction:column;gap:6px;max-height:min(56vh,560px);overflow:auto;padding-right:2px;scrollbar-gutter:stable}
      .df-budget-overview-row{width:100%;min-width:0;display:grid;grid-template-columns:10px minmax(0,1fr) auto auto;gap:5px 10px;align-items:center;padding:9px 0;border:0;border-bottom:1px solid #eef1f6;background:transparent;color:inherit;font-family:var(--ff);text-align:left;cursor:pointer}
      .df-budget-overview-row:last-child{border-bottom:0}
      .df-budget-overview-row:hover,.df-budget-overview-row:focus-visible{background:#fafbff;outline:0}
      .df-budget-overview-dot{width:8px;height:8px;border-radius:50%;background:#54bf8f}
      ...
      .df-budget-overview-main,.df-budget-overview-amount{min-width:0;display:flex;flex-direction:column;gap:2px}
      .df-budget-overview-main strong{color:#4b5669;font-size:10.5px;font-weight:850;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .df-budget-overview-main small,.df-budget-overview-amount small,.df-budget-overview-foot{color:#96a0b0;font-size:9px;line-height:1.35}
      .df-budget-overview-amount{text-align:right}
      .df-budget-overview-amount strong{color:#687386;font-size:10px;font-weight:850;white-space:nowrap}
      .df-budget-overview-state{min-width:76px;justify-self:end;padding:5px 8px;border-radius:999px;background:#eefaf5;color:#218464;font-size:9px;font-weight:850;text-align:center;white-space:nowrap}
      ...
      .df-budget-overview-track{grid-column:2 / -1;height:6px;border-radius:999px;background:#eef1f6;overflow:hidden}
      .df-budget-overview-track>i{display:block;height:100%;border-radius:999px;background:#54bf8f}
      ...
      .df-budget-overview-foot{margin-top:10px;padding-top:9px;border-top:1px solid #eef1f6}
      @media(max-width:920px){#pg-dashboard .invest-learn-bridge.df-invest-learning-quiet{grid-template-columns:1fr}#pg-dashboard .df-invest-learning-quiet .invest-learn-links{grid-template-columns:1fr}}
      @media(max-width:760px){#pg-dashboard .invest-learn-bridge.df-invest-learning-quiet{padding:14px;border-radius:14px}#pg-dashboard .df-invest-learning-quiet .invest-learn-links{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;overflow:visible;padding-bottom:0}#pg-dashboard .df-invest-learning-quiet .invest-learn-links>button{min-width:0;min-height:58px;grid-template-columns:1fr;padding:9px}#pg-dashboard .df-invest-learning-quiet .invest-learn-icon{display:none}#pg-dashboard .df-invest-learning-quiet .invest-learn-links strong{font-size:9.5px;line-height:1.25;white-space:normal}#pg-dashboard .df-invest-learning-quiet .invest-learn-links small,#pg-dashboard .df-invest-learning-quiet .invest-learn-links i{display:none}.df-budget-overview-summary{grid-template-columns:1fr 1fr}.df-budget-overview-summary>div:first-child{grid-column:1/-1}.df-budget-overview-row{grid-template-columns:10px minmax(0,1fr) auto}.df-budget-overview-state{grid-column:2 / -1;justify-self:start;min-width:0}.df-budget-overview-track{grid-column:2 / -1}}
    `;
    document.head.appendChild(style);
  }

  function scheduleBudgetOverview(delay = 90) {
